...
```
export GENTOO_MIRROR="http://distfiles.gentoo.org" && export GENTOO_ARCH="amd64" && export GENTOO_STAGE3="amd64" && export TARGET_DISK=/dev/sda && export TARGET_BOOT_SIZE=100M && export TARGET_SWAP_SIZE=1G && export GRUB_PLATFORMS=pc && export STAGE3_PATH_URL="$GENTOO_MIRROR/releases/$GENTOO_ARCH/autobuilds/latest-stage3-$GENTOO_STAGE3.txt" && export STAGE3_PATH=$(curl -s "$STAGE3_PATH_URL" | grep -v "^#" | cut -d" " -f1) && export STAGE3_URL="$GENTOO_MIRROR/releases/$GENTOO_ARCH/autobuilds/$STAGE3_PATH" && export LIVECD_KERNEL_VERSION=$(cut -d " " -f 3 < /proc/version) && export sfdisk ${TARGET_DISK} << END
